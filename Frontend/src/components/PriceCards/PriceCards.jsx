      ...
      <div className="w-full flex flex-wrap justify-center gap-3">
        {data &&
          data?.map((item, idx) => (
            ...
            >
              {item?.tag?.length > 0 && (
                <div className="absolute rounded-full text-white text-[12px] md:text-[14px] font-bold bg-gradient-to-r from-[#027adccc] to-[#123fa1] px-4 -top-2 -translate-y-2 left-1/2 -translate-x-1/2">
                  {item?.tag}
                </div>
              )}
              <div className="text-[22px] bg-gradient-to-r from-[#02AFDC] to-[#2563EB]  inline-block text-transparent bg-clip-text font-bold">{item?.title}</div>
